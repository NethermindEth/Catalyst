...
use web3signer::{Web3Signer, Web3SignerInfo};
...
pub async fn create_signer(
    web3signer_info: Option<Web3SignerInfo<'_>>,
    ...
    Ok(Arc::new(if let Some(web3signer_info) = web3signer_info {
        let address =
            web3signer_info.signer_address.parse().expect("signer address is required for web3signer usage");
        Signer::Web3signer(
            Arc::new(Web3Signer::new(web3signer_info).await?),
            address,
        )
