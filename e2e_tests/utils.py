...
def send_transaction(nonce: int, account, amount, eth_client, private_key):
    ...
    now = time.gmtime()
    current_time = (
        time.strftime("%H:%M:%S", now) + f".{int(time.time() * 1e6) % 1000000:06d}Z"
    )

    print(
        f"RPC URL: {eth_client.provider.endpoint_uri}, Sending from: {account.address}, nonce: {nonce}, time: {current_time}"
    )
...
def get_seconds_to_handover_window(beacon_client):
    ...
    if slot_in_epoch < 28:
        return (28 - slot_in_epoch) * int(
            beacon_client.get_spec()["data"]["SECONDS_PER_SLOT"]
        )
    else:
        return 0
...
def wait_for_slot_beginning(beacon_client, desired_slot):
    ...
    slots_to_wait = (
        number_of_slots_in_epoch - slot_in_epoch + desired_slot
    ) % number_of_slots_in_epoch - 1
    if (
        slots_to_wait < 0
    ):  # if we are in the desired slot, we need to wait for the next epoch
        slots_to_wait = number_of_slots_in_epoch - 1
    seconds_till_end_of_slot = seconds_per_slot - int(time.time()) % seconds_per_slot

    seconds_to_wait = (
        seconds_till_end_of_slot + slots_to_wait * seconds_per_slot + 1
    )  # +1 second to be sure we are in the next slot
...
def spam_n_txs_wait_only_for_the_last(eth_client, private_key, n, delay):
    ...
    for i in range(n):
        last_tx_hash = send_transaction(
            nonce + i, account, "0.00009", eth_client, private_key
        )
        time.sleep(delay)
    wait_for_tx_to_be_included(eth_client, last_tx_hash)
...
def wait_for_batch_proposed_event(eth_client, from_block, env_vars):
    ...
    for i in range(WAIT_TIME):
        ...
        time.sleep(1)
    assert False, (
        "Warning waited {} seconds for BatchProposed event without getting one".format(
            WAIT_TIME
        )
    )
...
def get_proposed_event_filter(eth_client, from_block, env_vars):
    if env_vars.is_shasta():
        ...
        return contract.events.Proposed.create_filter(from_block=from_block)
    else:
...
def wait_for_forced_inclusion_store_to_be_empty(l1_client, env_vars):
    ...
    while not forced_inclusion_store_is_empty(l1_client, env_vars):
        if i >= TIMEOUT:
            assert False, (
                "Error: waited {} seconds for forced inclusion store to be empty".format(
                    TIMEOUT
                )
            )
        time.sleep(1)
...
def spam_txs_until_new_batch_is_proposed(
    ...
    for i in range(number_of_blocks):
        spam_n_blocks(
            l2_eth_client, env_vars.l2_prefunded_priv_key, 1, env_vars.preconf_min_txs
        )
...
def stop_catalyst_node(node_number):
    ...
    result = subprocess.run(
        ["docker", "stop", container_name], capture_output=True, text=True, check=True
    )
...
def start_catalyst_node(node_number):
    ...
    result = subprocess.run(
        ["docker", "start", container_name], capture_output=True, text=True, check=True
    )
...
def wait_for_both_operators_active(
    l1_client,
    beacon_client,
    ...
    """Wait until both operators are active for the current epoch, retrying each epoch."""
    with open("../pacaya/src/l1/abi/PreconfWhitelist.json") as f:
        abi = json.load(f)
    ...
    for i in range(10):
        ...
        op1_active = contract.functions.isOperatorActive(
            operator1_address, epoch_timestamp
        ).call()
        op2_active = contract.functions.isOperatorActive(
            operator2_address, epoch_timestamp
        ).call()
        print(
            f"Epoch timestamp: {epoch_timestamp}, operator1 active: {op1_active}, operator2 active: {op2_active}"
        )
        if op1_active and op2_active:
            return
    assert False, "Timed out waiting for both operators to be active"


def get_operator_count(l1_client, preconf_whitelist_address):
    with open("../pacaya/src/l1/abi/PreconfWhitelist.json") as f:
        abi = json.load(f)
    contract = l1_client.eth.contract(address=preconf_whitelist_address, abi=abi)
    return contract.functions.operatorCount().call()


def remove_operator(
    l1_client, preconf_whitelist_address, operator_address, private_key
):
    with open("../pacaya/src/l1/abi/PreconfWhitelist.json") as f:
        abi = json.load(f)
    contract = l1_client.eth.contract(address=preconf_whitelist_address, abi=abi)
    account = l1_client.eth.account.from_key(private_key)
    nonce = l1_client.eth.get_transaction_count(account.address)
    tx = contract.functions.removeOperatorByAddress(operator_address).build_transaction(
        {
            "from": account.address,
            "nonce": nonce,
        }
    )
    signed_tx = l1_client.eth.account.sign_transaction(tx, private_key)
    tx_hash = l1_client.eth.send_raw_transaction(signed_tx.raw_transaction)
    receipt = l1_client.eth.wait_for_transaction_receipt(tx_hash)
    print(f"removeOperatorByAddress tx: {tx_hash.hex()}, status: {receipt.status}")
    assert receipt.status == 1, (
        f"removeOperatorByAddress transaction failed: {tx_hash.hex()}"
    )
    return receipt


def add_operator(l1_client, preconf_whitelist_address, operator_address, private_key):
    with open("../pacaya/src/l1/abi/PreconfWhitelist.json") as f:
        abi = json.load(f)
    contract = l1_client.eth.contract(address=preconf_whitelist_address, abi=abi)
    account = l1_client.eth.account.from_key(private_key)
    nonce = l1_client.eth.get_transaction_count(account.address)
    tx = contract.functions.addOperator(
    ...
    tx_hash = l1_client.eth.send_raw_transaction(signed_tx.raw_transaction)
    receipt = l1_client.eth.wait_for_transaction_receipt(tx_hash)
    print(f"addOperator tx: {tx_hash.hex()}, status: {receipt.status}")
    assert receipt.status == 1, f"addOperator transaction failed: {tx_hash.hex()}"
    return receipt


def get_current_operator_number(
    l1_client, l2_prefunded_priv_key, preconf_whitelist_address
):
...
def wait_for_epoch_with_operator_switch_and_slot(
    beacon_client, l1_client, preconf_whitelist_address, desired_slot
):
    ...
    for i in range(100):
        ...
        if current_operator != next_operator:
            break
    assert current_operator != next_operator, (
        "Current operator should be different from next operator"
    )
