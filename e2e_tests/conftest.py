...
from utils import (
    ...
    forced_inclusion_store_is_empty,
...
class EnvVars:
    ...
    def from_env(cls):
        ...
        if not l2_prefunded_priv_key:
            raise Exception(
                "Environment variable TEST_L2_PREFUNDED_PRIVATE_KEY not set"
            )

        l2_prefunded_priv_key_2 = os.getenv("TEST_L2_PREFUNDED_PRIVATE_KEY_2")
        if not l2_prefunded_priv_key_2:
            raise Exception(
                "Environment variable TEST_L2_PREFUNDED_PRIVATE_KEY_2 not set"
            )
        ...
        if not forced_inclusion_store_address:
            raise Exception(
                "Environment variable FORCED_INCLUSION_STORE_ADDRESS not set"
            )
        ...
        if not l1_private_key:
            raise Exception("Environment variable L1_PRIVATE_KEY not set")
...
def forced_inclusion_parameters(l1_client, env_vars):
    ...
    assert env_vars.l2_private_key != env_vars.l2_prefunded_priv_key, (
        "l2_private_key should not be the same as l2_prefunded_priv_key"
    )
...
def global_setup(l1_client, l2_client_node1, l2_client_node2, env_vars):
    ...
    while True:
        ...
        if (
            block_number_contract <= block_number_node1
            and block_number_contract <= block_number_node2
        ):
            break
