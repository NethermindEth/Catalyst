use crate::l1::{
    bindings::{
        BlobReference, MOCK_ECDSA_BIT_FLAG, Multicall, ProofType, ProposeInput, ProposeInputV2,
        RealTimeInbox, SubProof,
    },
...
impl ProposalTxBuilder {
    ...
    pub async fn build_propose_blob(
        ...
        if batch.user_ops.is_empty() && batch.l1_calls.is_empty() && inbox_calls.len() == 1 {
            ...
            let inbox_call = inbox_calls.into_iter().next().ok_or_else(|| {
                anyhow::anyhow!("inbox_calls unexpectedly empty after len==1 check")
            })?;
    ...
    fn build_l1_call_call(&self, l1_call: L1Call, bridge_address: Address) -> Multicall::Call {
        ...
        let call =
            bridge.processMessage(l1_call.message_from_l2.clone(), l1_call.signal_slot_proof);
